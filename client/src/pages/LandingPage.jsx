...
export default function LandingPage({ user, onLogout }) {
  ...
  // Languages data
  const languages = [
    { name: "English", native: "English", countryCode: "gb" },
    { name: "Hindi", native: "हिन्दी", countryCode: "in" },
    { name: "Gujarati", native: "ગુજરાતી", countryCode: "in" },
    { name: "Spanish", native: "Español", countryCode: "es" },
    { name: "French", native: "Français", countryCode: "fr" },
    { name: "German", native: "Deutsch", countryCode: "de" },
    { name: "Japanese", native: "日本語", countryCode: "jp" },
    { name: "Korean", native: "한국어", countryCode: "kr" },
    { name: "Chinese (Mandarin)", native: "中文", countryCode: "cn" },
    { name: "Arabic", native: "العربية", countryCode: "sa" },
    { name: "Portuguese", native: "Português", countryCode: "pt" },
    { name: "Russian", native: "Русский", countryCode: "ru" },
    { name: "Italian", native: "Italiano", countryCode: "it" },
    { name: "Dutch", native: "Nederlands", countryCode: "nl" },
    { name: "Turkish", native: "Türkçe", countryCode: "tr" },
  ];
  ...
  useEffect(() => {
    const simulatorSequence = async () => {
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === 1 ? { ...m, translating: true } : m)),
      );
      ...
      setMessages((prev) =>
        prev.map((m) => (m.id === 2 ? { ...m, translating: true } : m)),
      );
      await new Promise((r) => setTimeout(r, 1500));
  ...
  return (
    <div
      id="hero"
      className="min-h-screen bg-slate-50 text-slate-800 font-body overflow-x-clip selection:bg-brand selection:text-white relative"
    >
