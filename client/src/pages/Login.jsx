...
import { ShieldCheck, User, Lock, ArrowRight, Sparkles, Globe2 } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#FDFDFD] relative overflow-hidden">
      ...
      <div className="w-full max-w-[480px] z-10">
        ...
        >
          <div className="inline-flex items-center justify-center w-20 h-20 bg-white rounded-3xl shadow-premium border border-slate-100 mb-8 relative group">
            ...
            <Globe2 className="w-10 h-10 text-brand animate-float" />
            <Sparkles className="absolute -top-2 -right-2 w-6 h-6 text-amber-400 opacity-0 group-hover:opacity-100 transition-opacity" />
          </div>
          <h1 className="text-4xl font-display font-extrabold tracking-tight text-slate-900 mb-3">
            Biz<span className="bg-clip-text text-transparent bg-gradient-to-r from-brand-600 to-brand-400">Insights</span>
          </h1>
          <p className="text-slate-400 font-medium tracking-wide uppercase text-[10px]">Real-time Multilingual Intelligence</p>
        </motion.div>
        ...
        >
          ...
          <div className="flex bg-slate-50 p-1.5 rounded-2xl mb-10 border border-slate-100">
            <button
              ...
              className={`flex-1 py-3 text-sm font-bold rounded-xl transition-all duration-300 ${
                isLogin ? 'bg-white text-brand shadow-sm scale-[1.02]' : 'text-slate-400 hover:text-slate-600'
              }`}
            >
              Access Portal
            </button>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-slate-900 text-white py-4 rounded-2xl font-display font-bold hover:bg-brand-600 transition-all duration-300 flex items-center justify-center gap-3 group relative overflow-hidden"
            >
              ...
              <span className="relative z-10">{isLoading ? 'Initializing...' : (isLogin ? 'Sign In' : 'Join the Network')}</span>
        ...
        <footer className="mt-16 text-center space-y-4">
          <p className="text-slate-300 text-[10px] font-bold uppercase tracking-[0.2em] mb-4">Secured by Biz Insights Enterprise</p>
